...
> **【重点 · 定时关怀】** 与 **`后端项目正文.md`** **§9.6 / §9.6.1 / §9.6.2** 配套的前端侧已就绪：**`Demo/src/api/ws.js`** 解析 **`remind_trigger`**：仅当 **`trigger_type`** 与 **`delivery_message`** **均非空** 时先 **`appendChatMessage("user", 「【主动关怀】」+ trigger_type)`**、再 **`appendChatMessage("ai", delivery_message)`**，与 MySQL **`chat_session`** 的 **`user_input` / `ai_reply`** 及 **`GET /api/chat-sessions`** 历史列表一致（**无**客户端兜底话术）；**`trigger_content`** 仍为情景描述（与 REST 一致）。**`Demo/index.html`** 中角色侧 **`chat-item.ai`** 为浅紫色渐变；用户消息为 **`chat-item.user`**。若服务端在 **`REMIND_TRIGGER_USE_TTS`**（或未设时 **`REMIND_TRIGGER_USE_MIMO`**）开启且合成成功，可再收到 **`chunk_audio`（`remind_audio: true`）** + **WAV**：此时 **仅** 设置 **`pendingAudioMeta`** 并入队播放（驱动口型），**不得**再调用 **`applyVisibleTextSegment`** 更新 **本轮流式** **`chat-item ai`**，避免关怀朗读被重复写入正在进行的流式气泡。每条聊天消息的 **`buildChatWsPayload`** 附带 **`scene_time`**（**`formatUserWorldTimeForChat()`**），与后端 **`remind_extract`**（**每轮**；**`remind_extract_turn_should_run`** 对合法用户恒为真）及 **`scene_time`** 解析一并写入抽取提示；**`trigger_time`** 由后端解析校正（详见 **`后端项目正文.md`** §9.6.2）。后台默认约 **每 60 秒**扫描到期项（**`REMIND_TRIGGER_SCAN_INTERVAL_SEC`**）。联调：**`Demo/index.html`** 齿轮展开 **「定时关怀」** → **「立即扫描到期项」**（**`remindTriggers.js`** → **`POST /api/remind-triggers/scan-now`**），须 **`/ws/chat`** 在线才可能收到投递气泡。
...
3. **驱动虚拟形象**：根据服务端下发的 **`live2d_tags`** 帧（**`text`** 内可选 **`#emotion#`** / **`#motion#`** 行；**`catalog.motion` 为空** 时通常仅有表情行）解析出标识名，调用 **`LAppModel`** 的 **`setExpression`** 与（若有）**`startMotionByBasename`**，与连接建立后 **`catalog`** 中列表一致（可与正文 **`chunk`** 任意先后顺序到达）。  
4. **登录与资源**：主页面 **`index.html`** 在 **`localStorage.live2d_info`** 无有效登录信息时 **`replace`** 到 **`Demo/src/pages/Login.html`**；登录成功后通常写入 **`user_id`、`username`、`httpBase`**（与 **`Demo/src/api/apiBase.js`** 读取逻辑一致）。已登录用户可按 **`user_id`** 拉取 **`GET /api/live2d-model-assets/packages`** 与逐文件索引，对 **`object_key`** 行调用 **`download-url`** 换 **presigned URL**，构建远程 manifest（见 **`main.js`** 中 **`ensureLive2dRemoteManifests`**）。**`assetUpload.html`** 上传 zip 时服务端**固定**整理为 **`expressions/`**（全部 exp3）与 **`motions/`**（仅 motion3）两目录后再入库（见 **`docs/功能/live2d/第一层资源规范化.md`**）。  
...
| **接口层** | **`apiBase.js`**：`VITE_API_BASE` / `live2d_info.httpBase` → **`getApiBase()`**；**`wsConfig.js`**：WebSocket 基址、`session`、`package`、`user_id`；**`ws.js`**：双通道连接、流式正文、**`live2d_tags`**、`chunk_audio`、TTS 队列与口型；**`renderChatHistoryRows` / `prependChatHistoryRows`** 渲染左侧历史气泡；**`chatSessions.js`**：**`GET /api/chat-sessions`**（**`page` / `size`**）；**`assetUpload.js`**：模型包列表与资源索引；**`storageFetchUrl.js`**：presigned URL 代理（**`VITE_DOWNLOAD_SHARED_OBJECT_BASE`**） |
...
- **`/ws/chat`** 文本帧：**`catalog`** 可打日志；**`chunk`** / **`chunk_audio`** 的可见正文拼接到输出区（若页面存在 **`#output`** 则同步 **`appendOutputWithHeightLimit`**；**不再**从 **`chunk`** 读取 `expression`/`motion`）；**`live2d_tags`** 的 **`text`** 可含 **`#emotion#`** / **`#motion#`** 中 **任意一侧或两侧**（无动作资源的包常见 **仅表情行**），由 **`applyLive2dTagText`** 解析后调用 **`applyChatLive2dActions`**（**`lappdelegate.js`** → **`LAppLive2DManager`**）；**`done`** 归档到聊天记录并清空流式区；**`error`** 展示错误信息。  
- **主对话 AI 气泡「正在等待中」占位**：用户发送成功后 **`main.js`** 在 **`appendChatMessage("user", …)`** 之后调用 **`beginAiReplyWaitingUi()`**，在 **`#chat-list`** 底部追加 **`chat-item ai`**，并加 **`chat-item--waiting`** / **`aria-busy`**，展示「正在等待中···」（样式见 **`index.html`**）。流式正文由 **`applyVisibleTextSegment`**（**`chunk`** / 非 **`remind_audio`** 的 **`chunk_audio`**）写入 **`currentAiReply`** 并触发 **`updateStreamingAiMessage`**：**一旦累积正文 `trim()` 后非空**（判定为已出现可见正文），即移除占位样式并改用气泡 **`textContent`** 做流式展示；到达 **`done`** / **`error`** 时仍会收口 **`aiWaitingPhase`** 与 **`streamingAiItem`**。**`live2d_tags`** 不写入聊天气泡正文，**不会**单独取消该占位。  
...
- **`remind_trigger`**（若后端启用定时关怀）：**`trigger_content`** 为**情景描述**（与 **`GET /api/remind-triggers`** 一致）；聊天列表先 **`user`** 气泡 **`【主动关怀】`+`trigger_type`**，再 **`ai`** 气泡 **`delivery_message`**（与 **`chat_session`** 落库一致）。**`trigger_type` 为空或 `delivery_message` 为空** 时前端 **不追加气泡**。**投递话术**生成（**`remind_delivery`**）侧拼装含 MySQL **`persona`**、Redis **瞬时记忆**、可选 **`user_profile`**（**§9.6.1**）；**入库抽取**（**`remind_extract`**）仅本轮正文 + 人设指称，**§9.6.2**。其后若出现 **`chunk_audio` 且 `remind_audio === true`**：**关怀朗读专用**，只走音频队列（**§附录 B**），**不**写入 **正在进行的主对话流式** **`chat-item ai`**（关怀正文已由上述 **`appendChatMessage("ai", …)`** 展示）。  
- **`/ws/tts`**：**`audio_chunk`** 元数据后接 **ArrayBuffer**，入队顺序播放 WAV。

工具栏 **「更新长期记忆」** 对 **当前登录用户 + 当前模型包** 调用 **`POST /api/long-memories/consolidate-now`**（**`longMemory.js`**），即时执行 **滚动合并**（增量摘要与库内概要合并为单段，或可压缩历史堆叠段），**不必等待**后台最短 **24 小时**间隔（仍需 **Ollama**；无新对话且无需压缩时可能 `updated: false`，见 **`后端项目正文.md`** §9.3 与附录 C **§5.1**）。

工具栏 **「更新用户画像」** 对 **当前登录用户** 调用 **`POST /api/user-profiles/consolidate-now`**（**`userProfile.js`**），合并近 **24h** 内全部对话中的称呼/偏好等（**跨模型包**），**不受**后台 24h 最短间隔限制（无对话时可能 `updated: false`）。
...
| **表情** | **`setExpression(expressionId)`**，id 为 model3 **Name** | **`live2d_tags`** 中 **`#emotion#`** 后为 catalog 规范化标识名，须属于该 **`package`** 下 **`catalog.expression`** |
| **动作** | **`startMotionByBasename`** 匹配 **`.motion3.json` 文件名无后缀** | 若 **`catalog.motion` 非空**：**`live2d_tags`** 中 **`#motion#`** 后与 **`catalog.motion`** 标识一致；**若为空**则无 **`#motion#`** / 客户端不播聊天驱动动作（模型仍可仅有待机等行为） |
...
- 绑定 **文本输入 / 发送**、**麦克风与语音识别**：识别过程中将 **`#chat-input`** 与当前 **`text`** 同步（便于展示与手动改字）；**句末终稿**（**`isFinal`**）仍会 **`sendChatMessage`** 自动上传，成功后清空输入框并 **`appendChatMessage("user")`** + **`beginAiReplyWaitingUi()`**。若麦克风仍处于录音/识别中且输入框已有非空文本，用户 **再次点击麦克风按钮** 会先按与普通「发送」相同逻辑 **`sendChatMessage`**，再停止 **`AudioRecorder`** 与 **`SpeechRecognition`**（占位与取消规则见 **§4.3**）。  
- 工具栏：**上传模型**、**上传音色**、**角色定义**（打开 **`characterDef.html`**）、**更新长期记忆**、**更新用户画像**、**定时关怀**（卡片内 **立即扫描到期项**，联调 **`scan-now`**）、**退出登录**（清除 **`live2d_info`** 并回登录页）。
...
| WebSocket 连接与消息；**`scene_time`**（**`formatUserWorldTimeForChat`**）随 **`buildChatWsPayload`** 上报；**`beginAiReplyWaitingUi`** / **`updateStreamingAiMessage`**（见 **§4.3** 占位逻辑） | `Demo/src/api/ws.js` |
...
| `assetUpload.js` | 模型包列表、资源索引、上传 zip（服务端固定规范化）、HitAreas 模板等 |
...
- **`chunk`**：JSON，**`content`** 为一段可见正文（**不含** `expression`/`motion`；表情动作走 **`live2d_tags`**）。
- **`live2d_tags`**：JSON，**`text`** 为一行或多行 **`#emotion#…`** / **`#motion#…`**（可仅占一行；实现见 **`Demo/src/api/ws.js`** **`applyLive2dTagText`**）。
- **`chunk_audio`**：JSON，除 `content` 外声明 **`index`、`size`**；**下一帧**必须为 **二进制**，且为合法 **RIFF/WAVE**，字节数与 `size` 一致。若带 **`remind_audio: true`**（定时关怀朗读），**`content` 不进入主对话流式的 `chat-item ai` 气泡**（关怀正文已由 **`remind_trigger`** 写入 **`chat-item.user` + `chat-item.ai`**，与 **`chat_session`** 对齐），但仍须在本帧设置 **`pendingAudioMeta`** 以便下一帧二进制入 **`enqueueAudioAndPlay`**。
...
| **`stop()`** | 先将 **`onResult`** 置空（避免 **`flush`** 关闭前仍收到下行帧导致重复回调），再发送 **`{"cmd":"flush"}`**，释放音频节点与 WebSocket |
...
1. **`onResult`** 每次带回非空 **`text`** 时写入 **`#chat-input`**，与 **键盘输入框** 共用同一发送入口语义。  
2. **`isFinal && text`** 时调用 **`sendChatMessage(text)`**（自动发送）；成功后清空输入框，后续与 **文本对话** 相同：**`/ws/chat`** → Ollama → TTS 等（见 **`后端项目正文.md`** 附录 B · 架构与时序图 §1）。  
3. 用户在 **录音/识别进行中** 若输入框已有内容（含上述同步的中间结果或自行修改后的文本），**再次点击 `#record-btn`**：先 **`sendChatMessage`**（与发送按钮一致），再 **`AudioRecorder.stop()`** + **`SpeechRecognition.stop()`**。
...
- **`motions/`** 仅存放 **`.motion3.json`** 身体动作；**全部** `.exp3.json`（含换装/道具类）均在 **`expressions/`** 并进入表情 catalog。
- **`motions/`** 无 body 动作文件时，不要在 `model3.json` 里写 `Motions`，或只写空对象，避免引用缺失文件。
- **`expressions/`** 中的每个 `*.exp3.json` 若要在运行时作为**可切换表情**列出，须在 **`model3.json` 的 `FileReferences.Expressions`** 中逐项登记（见下文）。
- **不存在 `wardrobe/`**；历史包经 **`POST .../upload-zip`** 或 CLI 规范化后仅保留 **`expressions/`** 与 **`motions/`**（见 **`docs/功能/live2d/第一层资源规范化.md`**）。
...
- 默认 **`playbackRate = 1`**（可通过 **`globalThis.__TTS_PLAYBACK_RATE__`** 覆盖；**`≠ 1` 会连带改变音高**）。  
...
| **`MIMO_TTS_USER_PROMPT`** / **`MIMO_TTS_CONTEXT`** | 后端并入 MiMo **user**，与导演叠加（详见 **`后端项目正文.md`** 附录 D 第 5 节 / 附录 G）。 |
| **`MIMO_TTS_WS_INCLUDE_DIRECTOR`** | `1`：附带人设导演；`0`：仅 **`MIMO_TTS_WS_USER_HINT`**。 |
...
| 路径 | 变更类型 |
|------|-----------|
| **`PY/router/wschat.py`** | 单次合成开关、标点默认、流水线 **`TTS_STREAM_PIPELINE_SLOTS`**、MiMo **`merge_env_user_prompts=True`**、文档字符串与注释。 |
