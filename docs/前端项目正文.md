...
> **【重点 · 定时关怀】** 与 **`后端项目正文.md`** **§9.6 / §9.6.1 / §9.6.2** 配套的前端侧已就绪：**`Demo/src/api/ws.js`** 解析 **`remind_trigger`**，气泡正文使用 **`delivery_message`**（**`trigger_content`** 为情景描述，与 REST 一致）；以 **`appendChatMessage("remind", …)`** 写入聊天列表（样式 **`chat-item.remind`**）；若服务端跟随朗读帧，可出现 **`chunk_audio`（`remind_audio: true`）** + **WAV**。每条聊天消息的 **`buildChatWsPayload`** 附带 **`scene_time`**（**`formatUserWorldTimeForChat()`**），与后端 **`remind_extract`** 使用的服务器时间一并写入抽取提示，供模型换算「几分钟后」等相对语义（**`trigger_time` 仍由后端抽取模型生成**，前端不传触发时刻）。联调 UI：**`Demo/index.html`** 左侧聊天栏 **齿轮展开** 后的 **「定时关怀」卡片** 内按钮 **「立即扫描到期项」**，由 **`Demo/src/main.js`** 调用 **`Demo/src/api/remindTriggers.js`** → **`POST /api/remind-triggers/scan-now`**（须保持 **`/ws/chat`** 已连接才可能收到投递气泡）。
...
- **`remind_trigger`**（若后端启用定时关怀）：**`trigger_content`** 为**情景描述**（与 **`GET /api/remind-triggers`** 一致）；展示文案取 **`delivery_message`**（投递当次 LLM 生成）。后端可按 **`USER_PROFILE_IN_CHAT`** 注入 **`user_profile`**（见 **`后端项目正文.md`** §9.6.1）。  
