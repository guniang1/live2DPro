...
**目的**：用户发言与助手回复落库后，**在满足节流条件时**由后台线程调用 **`extract_and_persist_reminders`**（**`PY/utils/remind_extract.py`**），判断是否应插入 **`remind_trigger`** 行，供调度扫描与 **`remind_delivery`** 投递。

**调用链**：**`PY/router/wschat.py`** 在本轮 **`_persist_raw_memory`** 成功并得到 **`session_id`** 后，若 **`remind_extract_turn_should_run(user_id, package_key, chat_session)`** 为真，再 **`asyncio.to_thread`** 调用 **`extract_and_persist_reminders(..., package_key=..., session_id=..., scene_time=scene_time)`**。其中 **`scene_time`** 取自本条 **`/ws/chat`** JSON 载荷（与主对话 **`scene_time`** 同源，一般为前端 **`zh-CN` `toLocaleString`**）。

**节流（实现）**：默认 **`REMIND_EXTRACT_EVERY_N_TURNS=2`**（第 **2、4、6…** 轮抽取）；Redis 键按 **`user_id` + 规范化 `package_key` + WebSocket `session` 指纹** 计数，TTL **`REMIND_EXTRACT_TURN_CTR_TTL_SEC`**（默认 **604800** 秒）。**`N=1`** 等价于每轮抽取。**Redis 不可用或计数异常**时为少漏提醒 **仍每轮** 调用抽取。
...
**上下文**：**【MySQL 当前人设绑定】**（是否命中 **`persona`**、`persona_id`、`package_key`）；**【当前模型人设】（MySQL）**（**`tone_style` / `character_desc`**）；**`long_memory.period_overview`**；Redis **瞬时记忆**（行前缀 **「当前模型角色」**）；本轮 **【用户】** + **【当前模型角色回复】**（与当前包人设为同人，非第三者）。**`trigger_content`** 须遵守 **角色指称**：与人设或用户专名一致；禁止仅用孤立「助手」「AI」；若用户混用「助手」与专名且指同一人须合并表述，禁止「专名与助手」并列成两人。
...
| 定时关怀从对话抽取入库（默认每 **2** 轮；**MySQL 人设绑定块**；`scene_time` + 服务器时刻；**`trigger_type` 严格四类否则跳过**；`trigger_time` 解析 / 相对分钟锚点 / 年份收回 / 日历顺延；**`trigger_content` 角色指称规则**） | `PY/utils/remind_extract.py` |
