...
> **【重点 · 定时关怀】** 本项目 **定时关怀（主动关怀）全链路已实现并可联调**：对话轮末异步抽取（**`PY/utils/remind_extract.py`**：向抽取 LLM 注入 **服务器系统时间** 与 **`scene_time`** 解析得到的 **对话语境参考时间**，**`trigger_time` 完全由模型推算**；服务端仅做字符串解析与日历顺延，详见 **§9.6.2**）、**`remind_trigger`** 表与 REST（**§9.6**）、**`remind_trigger_scheduler`** 常驻扫描（**`PY/main.py`** lifespan）、**`POST /api/remind-triggers/scan-now`**、投递瞬间话术（**`PY/utils/remind_delivery.py`**：可选 Ollama，可与主对话一致注入 **`user_profile`**）、**`/ws/chat`** 帧 **`remind_trigger`**、离线用户重连补发（**`flush_pending_reminders_for_connection`**），以及环境允许时的 **TTS**（如 **`chunk_audio`** + **`remind_audio: true`**）。**帧字段**：**`trigger_content`** 在 REST 与 WebSocket 中均为**情景描述**；生成台词见 WS 的 **`delivery_message`**；话术拼装见 **§9.6.1**。
...
1. **对话与驱动数据**：通过 WebSocket（主要为 **`/ws/chat`**）接收用户文本，调用 **Ollama** 完成「动作/表情决策」与「聊天回复」两类推理，并将结构化结果与流式正文下发浏览器。  
   ...
   **主对话 `system`**（**`_build_memory_for_model`** 首条基底来自 **`_chat_system_prompt_for_session`**）：除 **`PY/.env`** 中的全局聊天指令与可选护栏外，已登录用户还可注入 **【叙事与现实语境】**（前端可选上行 **`scene_location`**：当前背景逻辑名；**`scene_time`**：用户设备本地真实时间）、**MySQL `user_profile` 摘要块**（是否注入由部署开关控制，见 **`PY/utils/user_profile_refresh.py`** 与 **`PY/.env`** 注释）。**定时关怀**投递瞬间由 **`PY/utils/remind_delivery.py`** 生成对用户台词时，若启用 LLM 且 **`USER_PROFILE_IN_CHAT`**（**`chat_inject_enabled()`**，与主对话一致）未关闭、库内 **`user_profile`** 存在非空字段，则将 **`format_profile_for_chat_system`** 输出的同一 **【用户画像】** 段落拼入该次 Ollama 调用的 **User** 消息（与人设块、情景描述、`session_id` 对应单轮对话并列），见 **§9.6.1**。  
...
| **长期记忆 · 周期概要更新** | 后台按模块常量周期扫描：从 **`chat_session`** **最近 24 小时**窗口读入 → **Ollama** 写入 **`long_memory.period_overview`**（含修补/扩写等）→ **`upsert`** 并 **`write_long_memory_text`** 刷新 Redis。手动全量回填：`python -m live2d_db.long_memory_consolidator`。对指定用户+模型包可 **`POST /api/long-memories/consolidate-now`**，即时调用 **`consolidate_one`**，**不受**后台 **`_MIN_GAP_SEC`（约 24h）** 限制。 | `PY/live2d_db/long_memory_consolidator.py`、`PY/live2d_db/http_api.py` |
...
**关怀话术生成（实现）**：**`PY/utils/remind_delivery.py`** 中 **`generate_remind_delivery_message`** 在 **`REMIND_DELIVERY_USE_LLM`** 未关闭时调用 Ollama；**User** 消息顺序为：可选 **当前包人设块** → 可选 **【用户画像】** → 可选 **【当前对话瞬时记忆】**（Redis 瞬时 List，与主对话同 `(user_id, package_key)` 分桶，**不含**短期压缩层）→ **【关怀类型】** → **【情景详细描述】**（库内 **`trigger_content`**）→ **【关联对话原文】**（**`session_id` → `chat_session`**）。**System** 要求 **重新撰写**当场台词，以情景与瞬时对话为主轴。**REST** 响应中 **`trigger_content`** 与库内一致；**WebSocket** 帧中 **`trigger_content`** 同名同义，生成稿在 **`delivery_message`**；**REST** 的 **`delivery_message`** 恒为 **`null`**。
...
- **【服务器当前时间】**：抽取当次 **`datetime.now()`** 快照（本地系统时钟），与入库后 **`trigger_time` 是否到期**、**`_roll_trigger_time_to_future`** 等与服务器时刻相关的逻辑对齐。**部署侧应保持主机日期／时区正确**。  
- **【对话语境参考时间】**：若能解析本条消息的 **`scene_time`**（**`_parse_scene_time_string`**，兼容 **`zh-CN` `toLocaleString`** 常见格式）则为用户设备时刻；否则 **等同上述服务器当前时间**。提示词要求模型 **自行完成**「几分钟后」「明天」「某月某日」等到 **绝对触发时刻** 的换算；并约定 **公历年默认与参考时间的自然年一致**，除非对话明确「明年」「下一年」等——**代码侧不会对「误写成下一年」等算术错误做自动纠正**，依赖模型与提示词。  
- **`trigger_time` 仅取自模型 JSON**：不做正则相对时间覆盖、无「模型空列表时的兜底插入」。  
...
**日历类日期顺延**：保留 **`_roll_trigger_time_to_future`**：模型给出的 **无年份/错年份日历点若早于当前（含 `_SKEW`）**，自动滚到 **下一次「同月同日（+原有时分秒若可解析）」** 落在未来；**不会**把「已是未来但年份不合理」（如无依据写成明年）单独拉回。
...
| 定时关怀投递话术（LLM）、可选注入用户画像 | `PY/utils/remind_delivery.py` |
| 定时关怀从对话抽取入库（`scene_time` + 服务器时刻注入 LLM；`trigger_time` 由模型给出；粗粒度解析与日历顺延） | `PY/utils/remind_extract.py` |
...
- **调度与时间窗**：默认 **最近 24 小时**内会话；同一用户同一包 **最短约 24 小时**方可再次触发周期概要更新；后台约 **每 24 小时**扫描——常量 **`_SOURCE_WINDOW_SEC`** / **`_MIN_GAP_SEC`** / **`_INTERVAL_SEC`**；模型与地址仍共用 **`OLLAMA_MODEL`**、**`OLLAMA_HOST`**。
...
周期概要更新的扫描间隔、会话窗、规则压缩参数见 **`PY/live2d_db/long_memory_consolidator.py`** 顶部常量，不在此逐条列举环境变量。
