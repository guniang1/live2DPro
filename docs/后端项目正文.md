...
- **筛选（与第一层规范化目录一致）**：**`.exp3.json`** 仅当路径在 **`expressions/`** 下或 **`is_listed_in_model3=1`** 时进入表情候选（**`motions/`** 下不应有 exp3）；**`motion3`** 或 **`.motion3.json`** 且在 **`motions/`** 或 model3 登记 → 动作候选。  
...
| POST | `/api/live2d-model-assets/upload-zip` | 上传模型 zip；**固定**先第一层规范化（**`expressions/`** + **`motions/`**），再写 MinIO 与索引 |
| GET | `/api/live2d-model-assets/hit-areas-template?package_key=` | HitAreas 侧栏 JSON 模板（上传前参考） |
...
该流程对应“上传 → **可选第一层规范化** → 存储 → MySQL 入库 → 前端按需取临时 URL”，用于替代手工放置本地 `Resources` 目录。详见 **`docs/功能/live2d/第一层资源规范化.md`**。

1. 前端或管理端向 **`POST /api/live2d-model-assets/upload-zip`** 提交表单：`user_id`、可选 `package_key`、`model_zip`（**无 normalize 开关，必定规范化**）。  
2. 解压至临时目录 → **`normalize_zip_bytes`** 整理后重新打包：删除冗余 `textures/moca.*` 等；**全部** `.exp3.json` 归入 **`expressions/`** 并写入 model3 **`Expressions`**；**`.motion3.json`** 归入 **`motions/`**；修正 LipSync；可选合并 HitAreas。响应 **`normalized`**（成功恒为 true）、**`normalize_warnings`**、**`emotion_count`**（exp3 数）、**`motion_count`**（motion3 数）。  
3. 服务端校验 zip 合法性与路径安全（过滤空路径、`..`、`__MACOSX` 等）。  
4. 解析包内入口 **`*.model3.json`** 与 **`FileReferences`**，建立 `logical_name`、`motion_group`、`is_listed_in_model3`、`is_entry_model` 索引。  
5. 将 zip 内文件上传对象存储（MinIO/S3），记录 `object_key` 与 `public_url`。  
6. 清理该用户该包旧索引行，批量写入 **`live2d_model_asset`**。  
7. **`invalidate_live2d_catalog_cache`**；前端经 **`download-url`** 构建远程 manifest 加载资源。

关键模块：`package_normalize.py`、`hit_areas_templates.py`；CLI：`PY/scripts/normalize_live2d_package.py`。
