...
1. **对话与驱动数据**：通过 WebSocket（主要为 **`/ws/chat`**）接收用户文本；**主对话**与 **表情/动作专用模型** 分工如下，二者 **并行**（专用模型由 **`asyncio.create_task(_emit_live2d_tags_parallel_llm)`** 发起，不阻塞主流式）。  
   ...
   **主对话 `system`**（**`_build_memory_for_model`** 首条基底来自 **`_chat_system_prompt_for_session`**）：**`_chat_system_prompt`** **仅拼接**非空的 **`OLLAMA_CHAT_SYSTEM`**、**`OLLAMA_CHAT_OUTPUT_GUARD`**、**`OLLAMA_CHAT_REMIND_PREVIEW_BAN`**（代码内 **无** 默认内置话术；**角色本体**主要由会话中的 **【当前模型人设】**（MySQL **`persona`**）承担）。若需「勿剧透定时关怀」类禁令，请在 **`PY/.env`** 自行配置 **`OLLAMA_CHAT_REMIND_PREVIEW_BAN`**。**已登录用户**还可注入 **【叙事与现实语境】**（前端可选 **`scene_location`**：叙事地点/环境（与画面背景一致）；**`scene_time`**：用户设备本地真实时间）、**MySQL `user_profile` 摘要块**（是否注入见 **`PY/utils/user_profile_refresh.py`** 与 **`PY/.env`**）。**定时关怀**投递话术见 **`remind_delivery`**（**§9.6.1**）：**人设**等同源；可选 **【用户画像】** 注入规则与主对话一致。  
...
| **WebSocket 对话与 TTS 编排** | `/ws/chat` 接单轮用户消息；主对话 **`system`** 可含人设 + **可选用户画像** + 场景块 + 短/长期记忆（见 **`_build_memory_for_model`**）；**`ollama_chat_messages`** 不含 Live2D 标签拼块。表情/动作由 **`_emit_live2d_tags_parallel_llm`**（**`_live2d_tag_llm_user_content_sync`** + **`action_llm_parallel_full_system`**）并行请求，下行 **`live2d_tags`**。轮末写 Redis 后可 **按轮刷新 `user_profile`**，连接 **`finally`** 再 **断线刷新**（**`utils/user_profile_refresh.py`**）；聊天流式、切段入队；**多协程并行 TTS** + **`_tts_flush_ordered`** 保序（合并流时音频亦可走同连接 **`chunk_audio`**，以代码为准）。 | `PY/router/wschat.py`、`PY/utils/user_profile_refresh.py` |
...
**`_chat_system_prompt`** **仅拼接**非空的 **`OLLAMA_CHAT_SYSTEM`**、**`OLLAMA_CHAT_OUTPUT_GUARD`**、**`OLLAMA_CHAT_REMIND_PREVIEW_BAN`**（代码内无默认全文；**提醒预告禁令**若需要请写在 **`OLLAMA_CHAT_REMIND_PREVIEW_BAN`**）。完整会话 **`system`** 由 **`_chat_system_prompt_for_session`** 继续拼接 **【叙事与现实语境】**、**【当前模型人设】**（MySQL **`persona`**）等（见 **§1**）。
