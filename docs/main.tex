...
% ====================================================
\begin{cnabstract}
    随着高校心理健康教育需求的提升与虚拟数字人技术的普及，纯文本对话难以提供形象与语音反馈，而常见Live2D示例又缺少与会话、记忆、账户绑定的后端。针对课程演示与学生低压力情感交互练习这一对可展示、可复核的场景，本文设计并实现须登录的浏览器端Live2D数字人Web系统，围绕形象与动作、语音播报、分层记忆持久化与定时主动关怀四项能力展开；因不涉及诊断与治疗，系统明确不用于医疗或心理咨询，亦不替代专业人员服务。

    系统采用B/S架构，前端基于Vite与Live2D Cubism Web实现，负责角色渲染、交互操作及私有资源预签名加载；语音经语音识别网关对接云端DashScope Fun-ASR完成识别后，与文本输入汇合，统一经对话WebSocket接口上行至服务端。服务端以FastAPI为核心，采用Ollama双模型分工模式：主对话模型负责流式自然语言生成，动作决策模型输出结构化表情与动作字段，通过资源目录索引约束确保可执行性；模型ZIP上传固定经第一层规范化整理为\texttt{expressions/}下全部\texttt{.exp3.json}与\texttt{motions/}下仅\texttt{.motion3.json}的标准目录后再写入对象存储与索引。语音合成默认采用云端小米MiMo服务，保留本地GPT-SoVITS切换路径。数据持久化层由关系型数据库、Redis分层缓存及对象存储构成，服务端通过统一REST接口集返回统一格式的响应。

    记忆系统实现瞬时、短期、长期三级分层：因单次\texttt{system}长度受限，三层均折叠进三种记忆块，块内按瞬时、短期、长期排列并声明冲突优先级，本轮用户输入单独占一条\texttt{user}消息；瞬时层保留最近若干轮原文，短期层对挤出轮次规则压缩，长期层由后台任务滚动合并\texttt{period\_overview}并在注入时超长保留文尾。稳定用户信息写入用户画像，含\texttt{display\_name}字段，与角色人设分列并注明身份区分，以免模型把角色名与用户称呼混淆；画像由后台任务在近24小时有对话且距上次更新满24小时时跨包合并，主对话另以角色锚点、回复长度约束与上下文裁剪抑制人设漂移。主动关怀在会话落库后异步抽取日程写入\texttt{remind\_trigger}，由调度器在到期扫描后结合人设与画像生成话术并经\path{/ws/chat}推送，离线用户重连时补发。
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{@{} >{\raggedright\arraybackslash}p{1.15cm} >{\raggedright\arraybackslash}p{2.35cm} Y >{\raggedright\arraybackslash}p{3.55cm} Y @{}}
        ...
        FR-11       & Live2D资源上传与索引 & 上传模型ZIP；固定执行第一层规范化（\texttt{expressions/}表情、\texttt{motions/}动作）后入库。 & ZIP→规范化→package索引                              & 可列出包信息并删除       \\
...
呈现层选用Live2D Cubism Web：二维网格变形与参数曲线控制表情/姿态，配合\texttt{physics}、\texttt{motion}、\texttt{expression}等资源形成完整渲染链路\cite{live2d}。工程上模型包资源目录约定为\texttt{expressions/}（全部\texttt{.exp3.json}，写入\texttt{model3.json}的\texttt{Expressions}，供对话表情）与\texttt{motions/}（仅\texttt{.motion3.json}身体动作）；上传ZIP时由服务端第一层规范化整理后再入库。expression/motion标识限定于当前模型包目录索引\texttt{catalog}并由服务端校验，以避免模型输出不可执行资源。相较VRM及重型三维方案，Live2D算力占用更低、SDK成熟；相较纯SVG/CSS虚拟形象，口型与商用动作资产更丰富。
...
\begin{itemize}\setlength{\itemsep}{2pt}\setlength{\parsep}{0pt}
    \item \path{POST /api/live2d-model-assets/upload-zip}：通过ZIP包批量上传Live2D模型资源；上传前固定执行第一层规范化后再入库；
    \item \path{GET /api/live2d-model-assets/hit-areas-template}：返回HitAreas侧栏JSON填写模板；
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{@{} >{\raggedright\arraybackslash}p{3.35cm} >{\raggedright\arraybackslash}p{2.25cm} Y Y @{}}
        ...
        \path{POST /api/live2d-model-assets/upload-zip}  & ZIP批量入库     & Form：\path{user_id}；\path{model_zip}；可选\path{package_key}（上传前固定规范化）                  & 包键、\path{inserted_rows}、\path{normalized}、\path{normalize\_warnings}、\path{emotion\_count}、\path{motion\_count}等 \\
...
\subsection{ZIP 入库、第一层规范化与删除一致性}

\path{POST /api/live2d-model-assets/upload-zip}先将ZIP解压至临时目录，固定调用\texttt{normalize\_zip\_bytes}（内部执行\texttt{normalize\_package}，并用\texttt{assess\_package\_standardness}记录非标准项写入警告）整理后重新打包再入库；无请求参数或环境变量可跳过。规范化结果仅保留两个资源子目录：\texttt{expressions/}（表情，写入\texttt{Expressions}并进入表情\texttt{catalog}）与\texttt{motions/}（\texttt{.motion3.json}身体动作，以及经Ollama或启发式判为非表情的\texttt{.exp3.json}，后者不写入\texttt{model3}、不进入表情\texttt{catalog}）。实现位于\texttt{PY/live2d\_db/package\_normalize.py}与\texttt{PY/utils/expression\_classify.py}；可选侧栏\texttt{.live2d-hit-areas.json}或\texttt{PY/live2d\_db/data/hit\_areas/}模板合并\texttt{HitAreas}。响应体除导入统计外，另含\texttt{normalized}（成功恒为真）、\path{normalize\_warnings}、\path{emotion\_count}、\path{motion\_count}、\path{hit\_areas\_source}等字段，便于管理端展示整理摘要。

规范化完成后继续：路径安全校验、推断包名、解析入口\texttt{*.model3.json}与\texttt{FileReferences}、批量写MinIO、重建\texttt{live2d\_model\_asset}索引（含\texttt{is\_listed\_in\_model3}、\texttt{is\_entry\_model}、\texttt{logical\_name}、\texttt{motion\_group}）。删除包时移除对象与索引行，并清理依附\texttt{persona}及Redis catalog缓存，保证再次连接\path{/ws/chat}时索引与存储一致。工程说明见仓库\texttt{docs/功能/live2d/第一层资源规范化.md}。
