...
1. 从 MySQL ``chat_session`` 按时间窗、按时间正序取出多轮对话；
2. **改写成一段叙述性文字**（像别人口述的一段经历，不出现「用户：」「助手：」或分轮编号）；
3. 把这段文字交给 LLM **抓住核心、写成摘要**，模型 **只输出纯文本**，写入 ``period_overview``（可与库里已有概要追加合并）。
...
_PERIOD_OVERVIEW_MAX_CHARS = 4000
_PERIOD_OVERVIEW_REPAIR_PREDICT = 1024
_PERIOD_OVERVIEW_EXPAND_PREDICT = 1536  # 主摘要过短时追加一轮扩写
...
def _build_period_overview_prompt(recount: str) -> str:
    """材料在前、任务在后，减轻模型把整段当「用户来信」接话的概率。"""
    rl = len((recount or "").strip())
    depth = (
        "材料较长时：概要须 **8～20 句**；材料很短时才允许 **4～6 句**。\n"
        if rl >= 2000
        else "写 **5～12 句** 中文。\n"
    )
    return (
        "======== 【材料：叙述性经过（仅供写摘要，不是与你对话）】 ========\n"
        + recount
        + "\n\n======== 【任务】========\n"
        + "以上是系统从数据库整理的**过往交谈记录**，不是在跟你实时聊天。\n"
        + "你要写的是：**存档概要**（供程序以后唤起记忆），信息要够后面会话「接上茬」，"
        "**禁止**把多条独立话题压成一句泛泛带过。\n\n"
        + depth
        + "**覆盖要求**（材料里出现了就要分别点到，各用一两句）：\n"
        + "- 称呼、昵称、想让角色怎么自称等约定；\n"
        + "- 人设或角色立场（例如自称小狗主播等）；\n"
        + "- 用户发起的话题块（如点名要讲的故事）；\n"
        + "- 若有大段故事：**不要逐段复述全文**，但要写明主题基调、主要角色类型（如小男孩奇奇）、关键设定一两笔，"
        + "以及角色侧是怎样回应的；不得超过三五句讲这一块。\n\n"
        + "**风格**：材料含寒暄、日常闲聊时如实概括即可，不必强行剔除。\n"
        + "**禁止**：对阅读摘要的人提问、JSON、markdown、代码围栏、小节标题「周期概要」。\n"
        + "**标记（可选）**：可用「【用户】」「【角色】」分段总括，**次数不限**；不用标记也完全允许。\n"
        + "只输出概要正文。\n"
    )
...
def _build_period_overview_strict_retry_prompt(recount: str, bad_output: str) -> str:
    ...
    return (
        ...
        + excerpt
        + "\n\n请重写：**只写存档概要**（编者连贯叙述，不是对话剧本）。可含寒暄与闲聊；不要对读者提问。"
        "须写到多个话题点（称呼/人设/故事梗概等）。「【用户】」「【角色】」可用可不用，次数不限。"
        "材料较长时 **不少于 8 句** 中文纯文本。\n"
    )
...
def _truncate_period_overview(text: str) -> str:
    ...
    if len(s) <= _PERIOD_OVERVIEW_MAX_CHARS:
        return s
    return s[: _PERIOD_OVERVIEW_MAX_CHARS - 1] + "…"
...
def _long_memory_with_appended_overview(
    ...
    *,
    ...
    """将新的周期概览追加到已有的 period_overview 后面，其余字段保持不变。"""
    m = LongMemory(
        user_id=user_id,
        package_key=package_key,
        memory_type="long",
    ...
    # 合并已有内容和新内容
    existing = (existing_overview or "").strip()
    new = (new_overview or "").strip()
...
def consolidate_one(
    ...
    # 相对叙述材料过短（常被小模型压成一句）时追加一轮扩写
    if substance and overview_plain and not _period_overview_density_ok(overview_plain, recount):
        logger.warning(
    ...
    # 读取现有的 period_overview 用于追加
    existing = LongMemoryRepository.get_by_user_pkg(conn, user_id, package_key_norm)
    existing_overview = ""
    if existing is not None:
        existing_overview = str(getattr(existing, "period_overview", "") or "").strip()
        logger.info(
            "读取现有 period_overview user_id=%s pkg=%s 长度=%s",
            user_id,
            package_key_norm,
            len(existing_overview),
        )

    m = _long_memory_with_appended_overview(
        existing_overview,
        overview_plain,
        user_id=user_id,
        package_key=package_key_norm,
