...
import { LAppView } from './lappview.js';
...
export class LAppSubdelegate {
  ...
  release() {
    if (this._wheelEventListener && this._canvas) {
      this._canvas.removeEventListener('wheel', this._wheelEventListener);
  ...
  initialize(canvas) {
    ...
    this._wheelEventListener = (e) => {
      ...
      this._view.onWheel(localX, localY, e.deltaY);
    };
    this._canvas.addEventListener('wheel', this._wheelEventListener, {
      passive: false
    });
  ...
  /**
   * @param {boolean} [suppressTap] 为 true 时不触发模型 onTap（例如在画布按下后在聊天区松开）
   */
  onPointEnded(pageX, pageY, suppressTap = false) {
    this._captured = false;
    ...
    this._view.onTouchesEnded(localX, localY, suppressTap);
  }
